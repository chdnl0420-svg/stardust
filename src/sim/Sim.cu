...
const float* Sim::fieldDevicePtr(Field field) {
    ...
    if (field == Field::Light) {
        ...
        if ((wantGlow || wantNebula) && d.projA && d.projB) {
            ...
            // **반지름 6 은 격자 눈금을 못 지운다 — 후광이 사각형으로 보였다.**
            //
            // 격자가 128 칸이면 화면 1616 픽셀에서 한 칸이 12.6 픽셀이다. 별빛은 이미
            // 네 칸에 나눠 뿌리지만(`kScatterLight` 의 CIC), 그래도 별 하나가 밝히는
            // 것은 25 픽셀짜리 네모다. 반지름 6 은 75 픽셀까지 퍼뜨리는데 **그 안에
            // 칸이 여섯 개뿐이라 계단이 눈에 남는다** — 2026-08-17 사용자가
            // 「무거운 별이 주변을 밝게 할 때 사각형 모양으로 밝게 한다」,
            // 「사각형 단위로 하나씩 밝아진다」고 알린 것이 이것이다.
            //
            // 12 로 늘리면 퍼지는 폭 안에 칸이 열둘이라 이웃끼리 충분히 겹쳐 눈금이
            // 묻힌다. **비용은 두 배가 아니다** — 가로·세로로 나눠 도는 분리 가능한
            // 가우시안이라 O(radius) 이고, 격자 16384 칸 × 25 = 41만 회로 미미하다.
            const int radius = 12;
