...
__global__ void kPlace(float4* pos, float4* vel, int n, int preset,
    ...
    if (preset == 0) {                           // 우주 필라멘트 — 고르게 깔고 씨앗을 심는다
        ...
        //
        // 밖으로 나간 알갱이를 안으로 **되접어**(`r' = 2R − r`) 표면에 안 쌓이게 해 봤는데
        // (2026-08-20, 「속이 빈 구」의 원인을 여기로 의심했다) **숫자가 그대로였다** —
        // 막 깔았을 때 안쪽 비율이 1.15% 로 소수점까지 같았다. 변위 0.16 이 반지름 0.5 를
        // 넘기는 알갱이가 거의 없어서 이 자리가 애초에 거의 안 걸린다. 원인은 여기가
        // 아니라 암흑에너지였다(`App.cpp` 의 `darkEnergy` 주석). 되돌렸다.
        {
        ...
        // ── 공 크기 — 무늬는 그대로 두고 좌표만 줄인다 ──────────────────────
        //
        // 빅뱅으로 쓰던 자리다(0.12 로 접어 한 점에 모았다가 터뜨렸다). 지금은 **접지
        // 않고 여유만 준다**(0.90 → 반지름 0.45) — 속이 빈 껍질이 되는 것을 고치면서
        // 「다 자란 크기에서 시작」으로 바꿨다(App.cpp 의 `bigBangShrink` 주석).
        // 곱셈이라 무늬는 보존되므로, 값만 되돌리면 빅뱅 연출도 그대로 살아난다.
...
__global__ void kParticleRadial(const float4* pos, int n, float* outCnt, int bins) {
    ...
    const float4 p = pos[i];
    if (p.x < 0.f) return;
    // **3D 반지름으로 잰다(2026-08-20).** 예전에는 원반을 보느라 xy 로만 쟀는데,
    // 판이 공인 지금은 그러면 **껍질과 속이 찬 공이 구별되지 않는다** — 얇은 껍질도
    // xy 로 투영하면 가운데가 채워진 것처럼 보인다.
    //
    // 균일한 공이면 세 구간의 알갱이 수가 부피비 **1 : 7 : 19** 로 나온다(부피가 r³
    // 이므로 (1/3)³ : (8−1)/27 : (27−8)/27). 바깥이 그보다 많으면 껍질이다.
    const float fx = p.x - 0.5f, fy = p.y - 0.5f, fz = p.z - 0.5f;
    int b = (int)(sqrtf(fx * fx + fy * fy + fz * fz) / 0.5f * (float)bins);
    if (b >= bins) b = bins - 1;
    if (b < 0) b = 0;
