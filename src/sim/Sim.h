...
enum class Preset {
    ...
    Accretion,    // 넓고 차가운 원반 — 가스가 뭉쳐 소행성·행성·별이 자란다
...
struct SimConfig {
    ...
    // ── 천체 만들기 (가스 → 소행성 → 행성 → 별) ──────────────────────────
    // 가스가 충분히 빽빽해진 자리에 천체가 생기고, 그 천체가 주변 가스를 먹으며 자란다.
    // 먹힌 가스는 사라지고 그 질량이 천체로 옮겨 가므로 전체 질량은 보존된다.
    bool  bodiesEnabled        = false;
    // 씨앗이 생기는 문턱(평균 밀도의 몇 배). 낮게 잡으면 원반이 처음 수축하는 그 순간에
    // 판 거의 전체가 문턱을 넘어 슬롯 상한까지 한꺼번에 태어난다 — 자라는 과정이 아니라
    // 이미 다 자란 화면이 된다(2026-08-14 실측: 40 으로 두었더니 t=0.13 에 2046개 포화).
    float bodySeedDensity      = 70.0f;
    float bodyGrabCells        = 0.35f;  // 먹는 반경(격자 칸 단위). 무거울수록 더 넓어진다
    // 등급 경계는 **전체 알갱이 수에 대한 비율**로 잡는다.
    // 절대 개수로 두면 파티클 수를 바꿀 때마다 뜻이 달라진다 — 40만에서 「별」이던 덩어리가
    // 1000만에서는 전체의 0.04% 밖에 안 되는 티끌이다.
    // 실제 태양계에서 소행성과 지구, 지구와 태양은 각각 수만~수십만 배 차이가 난다.
    // 그 계층이 보이려면 별이 되는 것이 아주 어려워야 한다 — 경계를 낮게 잡았더니
    // 600 스텝 만에 천체 75개 중 73개가 별이 됐다(2026-08-14 실측).
    float bodyPlanetFrac       = 0.001f;  // 전체의 0.1% 를 먹으면 행성
    float bodyStarFrac         = 0.02f;   // 전체의 2% 를 먹으면 별
};
...
// 가스가 뭉쳐 만들어진 천체의 현황.
// 파티클은 모두 같은 질량이라 「뭉쳐서 커진다」를 스스로 표현하지 못한다.
// 그래서 뭉친 결과를 천체라는 별도 존재로 두고, 그것이 주변 가스를 먹으며 자라게 한다.
struct BodyStats {
    int   count     = 0;    // 지금 있는 천체 수
    int   asteroids = 0;    // 소행성 등급 (가장 가벼움)
    int   planets   = 0;    // 행성 등급
    int   stars     = 0;    // 별 등급 (가장 무거움)
    float heaviest  = 0.f;  // 가장 무거운 천체가 먹은 가스 알갱이 수
    int   merges    = 0;    // 지금까지 합쳐진 횟수
    int   shatters  = 0;    // 지금까지 부서진 횟수
};

// 화면에 천체 하나를 그리는 데 필요한 것.
struct BodyView {
    float x = 0.f, y = 0.f; // 0~1 정규화 좌표
    float mass = 0.f;       // 먹은 가스 알갱이 수
    float radius = 0.f;     // 같은 좌표계의 반지름
    int   grade = 0;        // 0 소행성 · 1 행성 · 2 별
};
...
public:
    ...
    // 가스가 뭉쳐 만들어진 천체 현황. 천체 만들기가 꺼져 있으면 전부 0.
    BodyStats bodyStats() const;
    // 천체를 호스트 배열로 복사하고 실제로 채운 개수를 돌려준다(화면에 그리는 용도).
    int  readBodies(BodyView* out, int maxCount) const;
