...
struct App {
    ...
    // 가드가 스스로 켠 절반 해상도. **알갱이를 덜어내기 전에 먼저 이것을 쓴다** —
    // 그림이 흐려지는 것은 되돌릴 수 있지만 알갱이를 잃는 것은 그렇지 않다.
    // 사용자 설정(`ui.halfResWhenBusy`)과 따로 두어, 가드가 사람의 설정을 덮어쓰지 않는다.
    bool  guardHalfRes = false;
