...
struct App {
    ...
    // **왼쪽 줌 막대가 요청한 확대량**(양수 = 확대). 프레임 루프가 이것을 읽어 카메라를
    // 옮기고 0 으로 되돌린다.
    //
    // UI 가 직접 카메라를 만지지 않는 이유: 확대는 원근에서 **카메라를 앞으로 옮기는
    // 것**이라(돌리 줌) `camRot`·`camPos`·시야각이 함께 얽힌다. 그 계산이 이미 휠 처리에
    // 있으므로, 막대는 「얼마나」만 말하고 「어떻게」는 한 곳에 둔다 — 둘로 나뉘면
    // 휠과 막대가 서로 다르게 움직이기 시작한다.
