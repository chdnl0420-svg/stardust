...
struct App {
    ...
    // **빛으로 시작한다(2026-08-17).**
    //
    // 밀도로 시작하면 사용자가 앱을 열었을 때 이 판이 만든 것의 절반이 안 보인다 —
    // 별의 색(온도)·펄서 깜빡임·블랙홀 강착원반은 **빛 모드에만** 있다. 실측에서
    // 아무 설정도 안 보내고 띄우니 화면이 천체 컬러맵의 남보라였고, 「작은 별·큰 별·
    // 중성자별·블랙홀을 눈으로 가른다」가 통째로 안 보였다.
    //
    // 사용자가 처음에 말한 것이 「실제 별이 핵융합해서 빛을 내는 것도 보고싶고」였다.
    // **round-25 와 같은 함정이다** — 그때는 장면이 별 형성을 안 켜서 별이 0개였고,
    // 스크립트가 매번 명시로 켜고 재는 바람에 스물다섯 바퀴 동안 못 봤다.
    // 밀도로 보고 싶으면 보드에서 언제든 바꾼다.
