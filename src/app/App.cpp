...
void ApplyPresetDefaults(SimConfig& cfg, Preset preset) {
    ...
    if (preset == Preset::Filament) {
        ...
        // **암흑에너지 — 이것이 없으면 판이 결국 가운데로 무너진다.**
        //
        // 팽창(H=84)이 탈출 속도(91)보다 느려 이 판은 닫힌 우주다. H 를 91 위로 올리면
        // 안 무너지지만 밀도가 계속 낮아져 **구조가 아예 안 생긴다**(H=86 실측).
        // 실제 우주가 그 딜레마를 푸는 방법이 암흑에너지다 — 중력은 가까울수록 세고
        // 이것은 멀수록 세서, **처음엔 중력이 이겨 구조가 자라고 나중엔 이쪽이 이긴다.**
        //
        // 33 은 r=0.4 에서 중력의 2.35 배인데, 실제 우주가 2.3 배(Ω_Λ 0.7 : Ω_m 0.3)다.
        // 초기(r=0.06)에는 0.008 배라 구조 형성을 방해하지 않는다.
