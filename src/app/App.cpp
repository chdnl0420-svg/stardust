...
void ApplyPresetDefaults(SimConfig& cfg, Preset preset) {
    ...
    if (preset == Preset::Filament) {
        ...
        // ── 빅뱅 — 한 덩어리에서 퍼진다 ────────────────────────────────────
        //
        // 판 전체 구조를 0.12 로 접어 모은 뒤 거기서 터뜨린다. **무늬는 그대로 두고
        // 좌표만 줄이므로** 팽창하면 접기 전 구조가 그대로 커진다 — 요동을 새로 만드는
        // 것이 아니라 이미 있는 요동이 자란다.
        //
        // 정확한 한 점으로는 안 모은다. 100만 개를 한 칸에 넣으면 이 판을 죽인 조건과
        // 같아진다(0.04 면 칸당 14230 개, 0.12 면 527 개).
        cfg.bigBangShrink = 0.12f;

        // 팽창 속도 — 접은 크기에서 판 크기까지 퍼질 만큼을 에너지 보존으로 냈다.
        // `½v² = GM(1/r₀ − 1/r₁)`, r₀=0.06 · r₁=0.40 → v=5.05, H=84.
        // **더 낮추면 오히려 나빠진다**(실측: H=55 는 작은 공으로 남아 구조가 안 여문다).
        // 퍼져야 그물이 될 공간이 있다.
        cfg.hubble = 84.0f;

        // 암흑에너지 — 이것이 없으면 팽창이 멎은 뒤 가운데로 무너진다(닫힌 우주).
        // 중력은 가까울수록 세고(1/r²) 이것은 멀수록 세서(r), **처음엔 중력이 이겨
        // 구조가 자라고 나중엔 이쪽이 이겨 안 무너진다.**
        // 33 은 r=0.4 에서 중력의 2.35 배인데 실제 우주가 2.3 배(Ω_Λ 0.7 : Ω_m 0.3)다.
        cfg.darkEnergy = 33.0f;

        // 경계를 구면으로. 고립 경계의 기본 처리(여섯 면에서 되튐)는 퍼져 나간 우주를
        // 정육면체로 보이게 한다 — 0.42 부터 바깥 속도를 깎아 부딪히는 순간을 없앤다.
        ...
        // 알갱이 — 퍼지면 점유셀이 186만까지 가 렌더가 무거워진다. 성능 가드가 스스로
        // 찾아간 값이 35만이라 그 아래로 시작한다(가드가 걸리면 판이 다시 깔려 빅뱅이
        // 되풀이된다).
