...
void ApplyPresetDefaults(SimConfig& cfg, Preset preset) {
    ...
    if (preset == Preset::Filament) {
        ...
        //
        // r₀ = 0.06(접은 반지름) · r₁ = 0.5(판 절반) · GM = 0.9 → v = 5.1, H = 86.
        // 이보다 크면 그냥 흩어지고, 작으면 도로 무너진다. 중력이 곧바로 감속을 시작하므로
        // 「빠르게 터졌다가 천천히 퍼진다」가 저절로 나온다 — 감속을 따로 넣지 않는다.
        cfg.hubble = 86.0f;
        ...
        //
        // 그래서 **기본값은 팽창이 보이는 쪽에 맞춘다**(0.2 → 팽창이 10 초쯤에 걸쳐 퍼진다).
        // 구조까지 보려면 설정의 배속을 올리면 된다(최대 4 배).
        cfg.timeScale = 0.2f;
        return;
    }
