...
void ApplyPresetDefaults(SimConfig& cfg, Preset preset) {
    ...
    if (preset == Preset::Filament) {
        ...
        // 허블 팽창. `H ~ sqrt(gravity)` 가 팽창과 중력이 맞서는 문턱이라(gravity 0.9 → 0.95)
        // 그 언저리를 실측으로 훑어 골랐다(2026-08-19, simTime 18 까지):
        //
        //   H       최대밀도        점유셀            판정
        //   0(고립)  7 → 1354       183만 → 5.8만    **전체가 한 점으로 무너진다**
        //   0.75     42 → 437       166만 → 148만    뭉치며 퍼진다
        //   1.1      46 → **1780**  160만 → 163만    뭉치며 퍼진다(밀도 4 배)
        //
        // **밀도는 오르는데 점유셀이 유지되는 것**이 그물의 증거다 — 한 자리로 모이면
        // 점유셀이 급락한다(위 첫 줄). 둘 다 그 조건을 만족하고, 밀도가 더 자라는 쪽을 쓴다.
        cfg.hubble = 1.1f;
