...
void App::guardPerformance() {
    ...
    //
    // **40 에서 50 으로 올렸다(2026-08-20).** 이 선은 안전장치가 아니라 체감 성능을 지키는
    // 선이다 — 카드를 실제로 지키는 것은 스텝 하나를 보는 위의 `dangerStepMs` 이고,
    // 드라이버 타임아웃도 프레임이 아니라 커널 하나에 걸린다. 그래서 여기를 빡빡하게 잡을
    // 이유가 없고, 빡빡해서 **보던 우주가 자꾸 처음부터 다시 시작됐다.**
    constexpr float BUDGET_MS   = 50.0f;   // 20 FPS (전에는 40 = 25 FPS)
    ...
    // **배속으로 스텝이 여러 번 돈 것은 카드가 버거운 것이 아니라 사용자가 그러라고 한 것이다.**
    //
    // 배속이 3 을 넘으면 한 프레임에 스텝이 세 번 돈다(`stepRepsFor`). 그러면 프레임 시간이
    // 그대로 세 배가 되는데, 여태 그 값을 그대로 예산과 견주었다 — 그래서 **배속을 올리면
    // 반드시 가드가 걸렸다.** 2026-08-20 로그가 그 자리다: 스텝은 11.3 ms 로 여유로운데
    // 프레임이 40.3 ms 라 알갱이가 100만 → 70만 → 49만 → 34만으로 세 번 깎였고, 그때마다
    // 보던 우주가 처음부터 다시 시작됐다.
    //
    // 스텝 한 번당 시간으로 견준다. 카드가 실제로 버거운지는 그 값이 말한다.
    const int   reps      = (stepsLastFrame > 0) ? stepsLastFrame : 1;
    const float perStepMs = frameMs / (float)reps;

    if (perStepMs > BUDGET_MS) overBudgetMs += frameMs;
    else                       overBudgetMs = 0.0f;
    if (overBudgetMs < PATIENCE_MS) return;

    // ── 1단계: 그림을 흐리게 한다 ────────────────────────────────────────
    //
    // **알갱이를 덜어내기 전에 이것을 먼저 쓴다.** 절반 해상도는 픽셀 수를 4분의 1로
    // 줄이고 되돌릴 수 있지만, 알갱이를 잃는 것은 되돌릴 수 없다(판을 다시 깔아야 한다).
    // 뒤집힘 방지는 렌더 쪽이 이미 한다 — 켜지는 선과 꺼지는 선을 벌려 두고 서른 프레임을
    // 유지한다(`RenderField::draw`). 여기서는 「쓸지 말지」만 정한다.
    if (!guardHalfRes && !ui.halfResWhenBusy) {
        guardHalfRes = true;
        fx::mark("버거워서 절반 해상도로 낮춤 (스텝당 %.1f ms 가 %.0f ms 넘게 이어짐). "
                 "그래도 모자라면 그때 알갱이를 덜어낸다",
                 perStepMs, PATIENCE_MS);
        overBudgetMs = 0.0f;
    ...
    if (lower < cfg.particleCount) {
        fx::mark("버거워서 낮춤: %d → %d (스텝당 %.1f ms 가 %.0f ms 넘게 이어짐)",
                 cfg.particleCount, lower, perStepMs, PATIENCE_MS);
        cfg.particleCount = lower;
        // **여기서 `ApplyAutoGrid` 를 부르지 않는다(2026-08-20).**
        //
        // 그것은 알갱이 수를 보고 격자와 소프트닝을 다시 고르는데, 격자가 한 칸이라도
        // 달라지면 코어가 그것을 구조 변경으로 보고 **버퍼를 다시 잡으며 판을 처음부터
        // 깐다.** 알갱이만 줄이면 코어가 커널 범위만 좁혀 판을 살리는데, 격자를 건드리면
        // 그 길이 막힌다(`Sim::reconfigure` 의 「커널 범위만 좁힘」 분기).
        //
        // 격자를 그대로 두면 칸당 알갱이가 줄어 소프트닝이 이상적인 값에서 조금 벗어나지만,
        // 보던 우주가 사라지는 것에 비하면 작은 값이다.
        guardCappedTo = lower;
    }
    overBudgetMs = 0.0f;
    guardCooldown = COOLDOWN_FRAMES;
...
void ApplyPresetDefaults(SimConfig& cfg, Preset preset) {
    ...
    if (preset == Preset::Filament) {
        ...
        //
        // **반지름을 반으로 줄였다(2026-08-20).** 0.90(반지름 0.45)일 때는 공이 경계
        // (`softBoundR` 0.47)에 붙어 있어, 조금만 퍼져도 알갱이가 벽에 닿았다 —
        // 사용자 보고: 「우주 벽에 계속 닿아서 표면에 질량이 모여있어」. 실제로 바깥 구간
        // 비율이 t=0 의 59% 에서 t=10 에 71% 로 늘었다.
        //
        // 0.45 면 반지름 0.225 라 경계까지 0.245 가 뜬다 — 퍼질 자리가 공 크기만큼 있다.
        // 대신 부피가 8분의 1 이라 밀도가 8 배가 되고, 그만큼 중력이 세진다. 아래
        // 암흑에너지를 같은 배로 올려야 균형이 맞는다(`Λ = GM/R³`).
        cfg.bigBangShrink = 0.45f;
        ...
        // 그대로 덮는다.
        //
        // **공을 반으로 줄이며 함께 올렸다(2026-08-20). 다만 계산대로가 아니다.**
        //
        // `GM/R³` 로 보면 R 이 절반일 때 균형은 8 배(0.05 → 0.40)여야 한다. 그렇게 넣었더니
        // **t=15.7 부터 안쪽이 0% 인 껍질**이 됐다 — 실측 균형은 8 배가 아니라 **2 배**였다.
        // 알갱이 수를 바꿀 때와 같은 이유다(위 표): 격자와 소프트닝이 고정이라 밀도가 8 배가
        // 되면 칸당 알갱이가 그만큼 늘고, 유효 중력이 점질량 식대로 따라오지 않는다.
        //
        // 실측(알갱이 100만 · 배속 4 로 t=20 까지 · 균일한 공이면 안쪽 40.6%):
        //
        //   Λ      t=20 안쪽        거동
        //   0.05   84% (계속 상승)  수축한다 — 결국 한 덩어리로 뭉친다
        //   0.10   64% (t=6 부터 고정)  **평형** ← 이것
        //   0.20   64% (t=6 부터 고정)  평형이지만 밀도가 낮다(2643 vs 3125)
        //   0.40   0%               껍질
        //
        // **짧게 보고 고르면 틀린다.** 0.40 을 고를 때는 t≈6 까지만 봤는데, 팽창은 그 뒤에
        // 드러났다. 이 값을 다시 손댈 때는 최소 t=20 까지 본다.
        cfg.darkEnergy = 0.10f;
