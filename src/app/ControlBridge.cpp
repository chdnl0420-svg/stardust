...
bool ControlBridge::poll(App& app, int viewW, int viewH) {
    ...
    if (cmd == "set") {
        ...
        // **UI 슬라이더와 같은 범위여야 한다.** 여기가 4.0 이고 UI 가 10.0 이라 어긋나
        // 있었다 — 제어 채널로 6 을 보내면 4 로 잘리는데 화면에는 6 이 떠 있었다.
        ...
        if (has(kv, "darkEnergy"))     app.cfg.darkEnergy = clampF(getFloat(kv, "darkEnergy", app.cfg.darkEnergy), 0.0f, 500.0f, app.cfg.darkEnergy);
        if (has(kv, "velDispersion"))  app.cfg.velDispersion = clampF(getFloat(kv, "velDispersion", app.cfg.velDispersion), 0.0f, 10.0f, app.cfg.velDispersion);
        if (has(kv, "softBoundR"))     app.cfg.softBoundR = clampF(getFloat(kv, "softBoundR", app.cfg.softBoundR), 0.0f, 0.49f, app.cfg.softBoundR);
