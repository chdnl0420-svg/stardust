...
__global__ void kSplatPoints(const float4* pos, const float4* vel, const float* temp,
    ...
    if (colorBy == 4) {
        ...
        if (p.w > 0.f) {
            ...
            const float iu   = __saturatef(L / fmaxf(nebulaIonMin, 1e-6f) - 0.5f);
            const float ionf = iu * iu * (3.f - 2.f * iu);
            if (ionf <= 1e-4f) return;
            L *= ionf;
            ...
            // **표시되는 이 값은 실제 가스 온도(HII 영역은 약 1만 K)가 아니다** —
            // 색을 얻기 위한 값이라는 것을 여기 적어 둔다.
            TK = 1500.f;
        }
        ...
        if (bh.n > 0) {
            for (int k = 0; k < bh.n; ++k) {
                ...
                // **원반은 공이 아니다 — 각운동량이 면을 정한다.**
                //
                // 전에는 `r² = dx²+dy²+dz²` 로 중심에서의 거리만 봤다. 거리가 같으면
                // 위든 아래든 옆이든 다 원반으로 세어져서, 블랙홀을 **공처럼 둘러쌌다**
                // (2026-08-17 사용자 지적). 방향을 안 보면 구가 나오는 것이 당연하다.
                //
                // 실제 원반이 평면인 이유는 떨어지는 물질에 각운동량이 있어서다. 곧장
                // 못 떨어지고 한 면 안에서 돌며, 면에 수직인 운동은 서로 부딪혀 상쇄돼
                // 얇아진다. 그래서 판정도 **면 안의 거리(원통 반경)와 면에서 벗어난
                // 높이를 나눠** 봐야 한다.
                //
                // **회전축은 z 로 둔다.** 이 판의 은하는 xy 평면에서 돌게 깔리므로
                // (`diskPoint`) 블랙홀에 떨어지는 물질의 각운동량도 대체로 z 축이다.
                // **한계**: 블랙홀마다 삼킨 물질의 각운동량을 실제로 누적해 축을 정하는
                // 것이 아니라 판 전체의 회전면을 빌려 쓴다 — 은하 면이 기울면 안 맞는다.
                ...
                // **두께는 반경에 비례한다.** 관측된 얇은 원반은 `h/R ≈ 0.01~0.1` 이라
                // 바깥이 안쪽보다 두껍다(중력이 약해져 덜 눌린다). 0.08 을 쓰면 지평선
                // 서른 배 자리에서 두께가 2.4 지평선이다. 안쪽에서 0 이 되면 그 자리가
                // 아예 안 그려지므로 지평선 절반을 바닥으로 둔다.
                const float hz = fmaxf(R * 0.08f, rs * 0.5f);
                if (fabsf(dz) >= hz) continue;
                const float x = R / rs;                    // 지평선 단위 원통 반경
