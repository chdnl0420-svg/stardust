...
__global__ void kShade(const float* rho, const float* tempSum, int G, uchar4* out, int W, int H,
    ...
    if (u >= 0.f && u < 1.f && v >= 0.f && v < 1.f) {
        ...
        if (cmapKind == 3 && tempSum) {
            ...
            // 함께 바뀌므로 이 경로에서만 쓴다.
            // **0.5(제곱근) → 0.75 (2026-08-18).** 사용자가 「입자들이 밝기의 차이가
            // 거의 없다」고 알렸다.
            //
            // 이 경로에는 압축이 **세 겹**으로 겹쳐 있다 — 위에서 `log(1+d·bright)` 로
            // 한 번, `x/(1+x)` 로 또 한 번 누르고, 여기서 제곱근으로 다시 누른다. 실제
            // 별의 광도 차이가 `M^3.5` 라 수천 배인데 그 셋을 지나면 수십 배로 줄어든다.
            //
            // 지수를 올리면 대비가 살고 바닥이 내려간다 — 둘이 한 값에 묶여 있어 완전히
            // 갈라지지 않는다. 0.75 는 t=0.8 에서 0.894→0.846 으로 밝은 쪽을 거의 안
            // 건드리면서 t=0.25 에서 0.5→0.354 로 어두운 쪽만 낮춘다. **그만큼 대비가
            // 벌어진다.** 더 올리면(1.0) 바닥이 통째로 사라져 켜진 픽셀이 6.4%→1.34% 가
            // 되던 그 상태로 돌아간다.
            const float tv = __powf(t, 0.75f);
...
// 블랙홀의 자리·지평선·속도·GM. **커널 인자로 값을 통째로 넘긴다** — 최대 여덟 개라
// 256 바이트고, 별도 버퍼를 잡아 매 프레임 채우는 것보다 싸다.
//
// 속도가 필요한 것은 **알갱이의 각운동량을 블랙홀 기준으로 재기 때문**이다. 블랙홀이
// 움직이는데 정지한 것으로 보고 재면, 함께 흘러가는 물질이 도는 것처럼 잘못 보인다.
struct BHDisk {
    float4 p[8];      // xyz = 자리, w = 지평선 반지름 rs
    float4 v[8];      // xyz = 속도, w = GM (= rs·c²/2, 케플러 각속도를 낼 때 쓴다)
    int    n = 0;
};

// 블랙홀 둘레 마찰 가열의 세기(점성 소산 계수).
//
// 물리 형태(`D ∝ Ω²`)는 각운동량에서 나오고, 이 하나가 그것을 이 판의 밝기 눈금에
// 얹는다 — 태양급 별 하나가 1.0 이다.
//
// **눈금의 근거**: 은하 회전 속도가 0.25 인 이 판에서 블랙홀 0.01 거리를 도는 물질은
// `Ω² = v²/r² ≈ 625` 다. 여기서 태양급 별 하나만큼(1.0) 밝아지도록 잡으면 1.6e-3 이고,
// 그 자리에 알갱이가 수백 개 모이면 합쳐서 눈에 띄는 원반이 된다.
//
// **멀어지면 저절로 어두워진다** — 회전 속도가 대체로 일정한 구간에서 `Ω² = v²/r²` 이라
// 거리의 제곱에 반비례한다. 0.1 거리면 100분의 1, 0.3 이면 900분의 1 이다. 그래서
// 블랙홀 가까이만 밝고, 그것이 실제 강착원반이 보이는 방식이다.
__device__ __constant__ float kDiskVisc = 1.6e-3f;

// 원반 온도의 기준 각속도(제곱). 위 밝기 눈금과 짝이다 — 블랙홀 0.01 거리를 회전 속도
// 0.25 로 도는 물질(`Ω² = v²/r² = 625`)을 관측된 초대질량 원반 안쪽(5만 K)으로 잡는다.
//
// **지평선에서 떼어내 둔 것이 중요하다.** 전에는 최소 안정 궤도(3 r_s)를 기준으로 삼았는데,
// `r_s` 는 질량과 광속에 따라 몇 자릿수씩 움직이는 값이라 그때마다 색이 통째로 무너졌다.
__device__ __constant__ float kDiskOmRef = 625.0f;
...
__global__ void kSplatPoints(const float4* pos, const float4* vel,
    ...
    if (colorBy == 4) {
        ...
        // ── 블랙홀 둘레의 마찰 가열 ────────────────────────────────────────
        //
        // **블랙홀은 안 보인다. 보이는 것은 그 둘레를 도는 물질이 마찰로 데워진 빛이다.**
        //
        // 2026-08-18 에 여기 있던 것을 통째로 걷어냈다. 그것은 「블랙홀에서 거리 r 이면
        // 이만큼 밝게 칠한다」로 **원반을 그려 넣는** 코드였고, 세 가지가 부자연스러웠다 —
        // 사용자가 「보이지 않는 구형 막같은게 생겨서 그 경계에 입자가 붙어있는 모양」
        // 이라고 알린 것이 그것이다.
        //
        //   · `if (R2 >= rOut*rOut) continue` 와 `if (|dz| >= hz) continue` — **하드 컷
        //     두 개.** 경계에서 밝기가 절벽처럼 0 이 되니 없는 막이 보인다.
        //   · 그 안에 **있기만 하면** 빛났다. 마찰과 무관해서, 곧장 떨어지는 물질도
        //     도는 물질과 똑같이 밝았다.
        //   · 회전축을 z 로 고정하고, `grown`(삼킨 개수/12만)이라는 인위적 계수를 곱했다.
        //
        // **대신 실제로 일어나는 일을 잰다.** 원반이 빛나는 이유는 안쪽과 바깥쪽이 다른
        // 각속도로 돌아(차등회전) 이웃한 층끼리 쓸리기 때문이다. 그 전단이 점성으로
        // 열을 내고, 단위질량당 소산률은
        //
        //     D ∝ (r dΩ/dr)² ∝ Ω²          (케플러에서 r dΩ/dr = −(3/2)Ω)
        //
        // 이다. 그리고 Ω 는 **그 알갱이의 실제 각운동량**에서 바로 나온다:
        //
        //     Ω = |L⃗| / r² ,   L⃗ = r⃗ × v⃗   (블랙홀 기준 상대 위치·속도)
        //
        // 이 한 줄이 앞의 문제를 전부 없앤다:
        //   · **하드 컷이 필요 없다.** Ω² 은 r 이 커지면 부드럽게 0 으로 간다(케플러면 r⁻³).
        //   · **평면 판정이 필요 없다.** 각운동량 벡터가 곧 회전면이라 원반이 저절로 선다.
        //     축을 z 로 고정하지 않으므로 기울어진 원반도 제대로 나온다.
        //   · **도는 것만 빛난다.** 곧장 떨어지는 물질은 L=0 이라 0 이다 — 실제로도
        //     각운동량 없이 떨어지는 물질은 원반을 못 만들고 그냥 삼켜진다.
        //   · **`grown` 같은 계수가 없다.** 갓 생긴 블랙홀 둘레에 아직 도는 물질이 없으면
        //     저절로 어둡고, 물질이 모여 돌기 시작하면 저절로 밝아진다.
        //
        // 케플러 궤도에서 Ω² = GM/r³ 이므로 표준 원반의 `D ∝ r⁻³` 이 그대로 재현된다 —
        // 식을 그렇게 적은 것이 아니라 각운동량에서 나온다.
        //
        // **비용**: 알갱이당 블랙홀 수만큼 벡터곱 한 번. 여덟 개면 100만 알갱이에 800만
        // 회이고 나눗셈은 원반 안일 때만 한다. 블랙홀이 없으면 루프를 통째로 건너뛴다.
        if (bh.n > 0) {
            const float4 pv = vel[i];
            for (int k = 0; k < bh.n; ++k) {
                const float4 b  = bh.p[k];
                const float4 bv = bh.v[k];
                ...
                // 각운동량 L⃗ = r⃗ × v⃗ (단위질량당).
                const float lx = ry * vz - rz * vy;
                const float ly = rz * vx - rx * vz;
                const float lz = rx * vy - ry * vx;
                const float L2 = lx * lx + ly * ly + lz * lz;
                if (L2 <= 0.f) continue;                   // 곧장 떨어진다 — 원반이 아니다
                // Ω² = L²/r⁴.
                const float om2 = L2 / (r2 * r2);
                // 소산률. 계수는 이 판의 밝기 눈금(태양급 별 = 1.0)에 맞춘 값이다.
                //
                // (최소 안정 궤도(3 r_s) 안쪽을 매끄럽게 줄이던 항을 지웠다 — 2026-08-18.
                //  지평선을 실제 크기로 되돌리자 `3 r_s = 1.8e-6` 이 되어 **모든 알갱이가
                //  그 바깥**이라 항상 1 이었다. 이 눈금에서 원반의 안쪽 끝을 정하는 것은
                //  ISCO 가 아니라 중력 소프트닝(격자 세 칸)이다 — 그보다 안쪽으로는
                //  알갱이가 들어갈 수 없다.)
                const float Ld = kDiskVisc * om2;
                if (Ld <= 1e-6f) continue;
                // **온도도 같은 물리에서 나온다.** 복사가 흑체면 `D ∝ T⁴` 이므로
                // `T ∝ D^(1/4) ∝ Ω^(1/2)` 다 — 표준 원반의 `T ∝ r^(-3/4)` 가 여기서 나온다.
                //
                // **기준점을 지평선에서 떼어냈다(2026-08-18).** 전에는 `Ω²(3 r_s)` 를 기준으로
                // 삼았는데, 지평선이 실제 크기가 되자 그 값이 5.15e14 로 뛰어 실제 알갱이의
                // Ω²(≈25)와 비가 4.9e-14 가 됐다 — 클램프에 걸려 **모든 원반이 500 K 한 색**
                // 이었다. 기준은 이 판에서 실제로 나오는 값이어야 한다: 블랙홀 0.01 거리를
                // 회전 속도 0.25 로 도는 물질이 `Ω² = v²/r² = 625` 이고, 그 자리를 관측된
                // 초대질량 원반 안쪽(5만 K)으로 잡는다. 물리 형태(`T ∝ Ω^½`)는 그대로다.
                const float Td = 50000.f * __powf(fmaxf(om2 / kDiskOmRef, 1e-6f), 0.25f);
    ...
    if (!devPixels_) {
        ...
        if (wantPoints) {
            ...
            BHDisk bhDisk;
            {
                ...
                // **GM 은 실제 질량에서 낸다 — 지평선에서 역산하면 안 된다.**
                //
                // `rs = 2GM/c²` 로 되돌리려다 1000배 틀렸다(2026-08-18). 이 판의 `rs` 는
                // 물리 공식대로가 아니라 **화면에서 세 원이 구분되도록 부풀린 값**이라
                // (`setRsFrom` 주석) 그 관계가 성립하지 않는다. 실측: rs 로 역산한 GM 이
                // 0.4, 실제 `G·M` 은 5.3e-4 였다.
                //
                // 블랙홀의 `mass` 는 삼킨 **개수**이므로 총 개수로 나눠 총질량 1 눈금에 맞춘다.
                const float G   = app.sim.config().gravity;
                const int   nAll = app.sim.particleCount();
                const float invN = 1.0f / (float)(nAll > 0 ? nAll : 1);
                for (int k = 0; k < bhN && bhDisk.n < 8; ++k) {
                    const BlackHoleState s = app.sim.blackHoleAt(k);
                    if (!s.active) continue;
                    const int j = bhDisk.n++;
                    bhDisk.p[j] = make_float4(s.x, s.y, s.z, s.rs);
                    bhDisk.v[j] = make_float4(s.vx, s.vy, s.vz, G * s.mass * invN);
                }
            }
