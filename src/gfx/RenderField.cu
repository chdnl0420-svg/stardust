    ...
    if (colorBy == 4) {
        ...
        if (bh.n > 0) {
            ...
            for (int k = 0; k < bh.n; ++k) {
                ...
                // **최소 안정 궤도(3 r_s) 안쪽에는 원반이 없다.** 거기서는 안정된 원궤도가
                // 없어 물질이 나선으로 빨려 든다 — 그것이 원반의 안쪽 끝을 정하는 실제
                // 경계다. 컷이 아니라 매끄럽게 줄여 안쪽 가장자리가 깎이는 것으로 보인다.
                const float isco = 3.0f * rs;
                const float rr   = __fsqrt_rn(r2);
                const float fIn  = (rr < isco) ? (rr / isco) * (rr / isco) : 1.0f;
                // 소산률. 계수는 이 판의 밝기 눈금(태양급 별 = 1.0)에 맞춘 값이다.
                const float Ld = kDiskVisc * om2 * fIn;
                if (Ld <= 1e-6f) continue;
                // **온도도 같은 물리에서 나온다.** 복사가 흑체면 `D ∝ T⁴` 이므로
                // `T ∝ D^(1/4) ∝ Ω^(1/2)` 다. 최소 안정 궤도에서의 각속도를 기준으로
                // 잡아 그 자리가 5만 K(관측된 초대질량 원반 안쪽)가 되게 맞춘다.
                const float gm      = fmaxf(bv.w, 1e-12f);
                const float omIsco2 = gm / (isco * isco * isco);   // Ω² = GM/r³
                const float Td = 50000.f * __powf(fmaxf(om2 / omIsco2, 1e-8f), 0.25f);
