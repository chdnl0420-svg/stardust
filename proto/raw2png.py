"""밀도 격자 raw(float32) -> PNG.  외부 이미지 라이브러리 없이 zlib만으로 PNG를 쓴다."""
import zlib, struct, sys, os
import numpy as np

G = 1024
OUT = os.environ.get("NBODY_OUT", r"D:\Project\Nbody\proto\out")
...
def load(name):
    ...
    if a.size != G * G:
        raise RuntimeError(f"{name}: 크기 불일치 {a.size} != {G*G}")
