...
- [x] 시간 배속을 조절해 시뮬레이션이 빨라지고 느려지는 것을 본다 — **round-07 회복**: 내리는 쪽은 CFL 한계를 그 비율로 낮추고(`Sim.cu` `slowFactor`), 올리는 쪽은 프레임당 스텝 반복(`App::tick` 의 `reps`). 회귀테스트 [11] **배속0.25 simTime=0.009093 vs 1.0=0.034053 (3.75배)**, 브릿지 [2] dtUsed 0.000101 → 0.000025, [3] 프레임당 스텝 1 → 3
...
### 중력
- [x] 중력 세기를 조절해 뭉치는 정도가 달라지는 것을 본다 — **round-07 회복**: `App::tick` 이 매 프레임 `sim.reconfigure(cfg)` 로 넘긴다. 브릿지 테스트 [1] **프리셋을 거치지 않은 set 단독 변경에서 최대속력 1.6847 → 5.0529 (3.00배, 이론 √9=3배)**
...
- [x] 소프트닝을 조절해 근거리 거동이 달라지는 것을 본다 — **round-07 회복**: 보드 전달 경로 + `kPoissonPeriodic` 에 주파수공간 가우시안 감쇠 추가. 회귀테스트 [13] **주기 경계에서 소프트닝 0.5셀 최대속력 0.000316 vs 6.0셀 0.000175 (1.81배)**. 고립 경계는 `kGreen` 의 `eps`
- [x] 격자 중력이 직접 O(N²) 대비 허용 오차 안에 있다 — 확인: `testForceAccuracy` **실측 0.0729(256²) / 0.1200(512²)**, 판정선 0.15

### 가스
- [x] 압력을 켜고 꺼서 뭉침 정도가 달라지는 것을 본다 — 물리는 QA D1 압력off 2585.8 → on 89.08. **round-07 에서 보드 전달 경로 회복**(`App::tick` 의 `sim.reconfigure(cfg)`)
...
- [x] 온도 추적을 켜서 충돌면이 달아오르는 것을 색으로 본다 — QA D2 평균온도 0.4730, `round-06-qa/11-temperature.png`. **round-07 에서 보드 전달 경로 회복**
- [x] 복사 냉각을 켜서 가스가 식으며 더 뭉치는 것을 본다 — QA D3 평균온도 냉각off 0.4629 → on 0.0554 (8.4배). **round-07 에서 보드 전달 경로 회복**
- [x] 별 형성을 켜서 조밀·차가운 가스가 별로 바뀌는 것을 본다 — **round-07 회복**: `kReorder`·`kCompact` 가 `isStar` 를 함께 옮기고, 지운 뒤 꼬리 슬롯의 표식을 비운다. 회귀테스트 [12] **별 199,101/200,000 → 판 전체를 지운 뒤 0/0**. 개수는 QA D4 별 1,994,530 / 2,000,000
- [x] 단열지수 γ 를 조절해 압력 반응이 달라지는 것을 본다 — QA D5 최대밀도 γ=1.1 135.75 vs γ=2.0 104.77. **round-07 에서 보드 전달 경로 회복**

### 우주
- [x] 우주 팽창을 켜서 구조 형성이 팽창과 경쟁하는 것을 본다 — QA E1 점유셀 팽창off 1,008,059 → on 1,008,070, `round-06-qa/14·15-web-*.png`. **round-07 에서 보드 전달 경로 회복**
...
- [x] 컬러맵을 천체/흑백/열화상으로 바꾼다 — **round-07 회복**: `kSplatPoints` 가 `cmapKind` 를 받는다. 점 렌더 화면 평균색 실측 — 천체 R180/G139/**B205**(보라), 흑백 R202/G202/B203(무채색), 열화상 R205/G200/**B8**(주황). `round-07-qa/points-cmap-*.png`
...
- [x] 프리셋을 고르면 경계·압력·팽창이 함께 바뀐다 — **round-07 회복**: 보드 버튼을 `ApplyPresetDefaults → applyConfig → reset` 순서로 바로잡았다(`Board.cpp` 「순서가 중요하다」 주석). 브릿지 테스트 [5] **web=periodic/100만 · empty=isolated/0개 · spiral=isolated/100만**
